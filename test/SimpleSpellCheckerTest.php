...
class SimpleSpellCheckerTest extends TestCase
{
    public function testSpellChecker()
    {
        ...
        $simpleSpellChecker = new SimpleSpellChecker($fsm);
        for ($i = 0; $i < count($modified); $i++) {
            $this->assertEquals($original[$i], $simpleSpellChecker->spellCheck($modified[$i]));
        }
    }
}